    ...
    <ol className="space-y-3">
      {cycles.map(({ id, data }) => {
        ...
        return <li key={id} className="rounded-2xl border border-line bg-surface p-4">
          ...
          {data.status === 'upcoming' && !isReadOnly && <div className="mt-3 flex flex-col items-stretch gap-1 border-t border-line/60 pt-3"><Button className="w-full px-3 py-2 text-sm" onClick={() => void start(data.cycleNumber)} disabled={busyCycle !== null || startBlockReason !== null} title={startBlockMessage ?? undefined}><Play size={15} weight="fill" />{busyCycle === data.cycleNumber ? t('dash.starting') : t('workspace.startCycle')}</Button>{startBlockMessage && <p className="text-center text-xs text-muted">{startBlockMessage}</p>}</div>}
        </li>
      })}
    </ol>
  </>
