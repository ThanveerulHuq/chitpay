    ...
    <Page>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted">
                  {cycle ? `Due ${cycle.dueDate}` : `From ${g.startDate}`}
                </p>
              </div>
            </div>
            <Chip tone={overdue && myStatus !== 'paid' ? 'overdue' : myStatus === 'paid' ? 'paid' : 'pending'}>
              {myStatus === 'paid' ? 'Paid' : overdue ? 'Overdue' : 'Pending'}
            </Chip>
          </div>
          <p className="mt-3 text-sm text-muted">
            Pay the admin directly — this screen updates once they record it.
          </p>
        </div>
      )}
      ...
      {cycle?.recipientMembershipId && board && (
        <div className="mt-4 flex items-center gap-3 rounded-2xl bg-accent-soft p-4">
          ...
          <p className="text-sm font-semibold text-accent-strong dark:text-accent">
            {board.find((e) => e.membershipId === cycle.recipientMembershipId)?.name ?? 'A member'}{' '}
            receives{' '}
            {formatMinor(
      ...
      {board && (
        <section className="mt-6">
          <h2 className="mb-3 text-sm font-semibold text-muted">This month's payments</h2>
